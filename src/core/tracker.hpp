...
class Tracker {
  ...
  private:
    ...
    vector<cv::DMatch> knnmatch(cv::Mat& descriptors1, cv::Mat& descriptors2);

  public:
    explicit Tracker(Matcher matcher, float knn_threshold, int norm_threshold, int norm_type);
